...
# --- SERVIDOR WEB FLASK (Para mantener a Lumen vivo 24/7 con UptimeRobot) ---
app = Flask("")
  ...
  async def confirm(
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f" individual:**\n🪙 {rewards['soles']} Soles | 🏆 {rewards['copas']}"
            f" Copas | 🙏 {rewards['favor']} Favor\n✨ {rewards['exp']} EXP"
        ),
...
async def ayuda(interaction: discord.Interaction):
  ...
  embed.add_field(
      name="📜 Personajes",
      value=(
          "`/crear-personaje` | `/mis-personajes` | `/perfil` | `/comprar-slot`"
          " | `/clan`"
      ),
      inline=False,
  )
  ...
  if interaction.user.guild_permissions.administrator:
    embed.add_field(
        name="🔮 Administración y Masters",
        value=(
            "`/mision-recompensa` | `/dar-item` | `/evento-crear` |"
            " `/master-rol` | `/tienda-rol`"
        ),
        inline=False,
    )
...
@bot.tree.command(
    ...
    description=(
        "Crea un nuevo personaje (Máximo 3 base + 2 adicionales comprados)"
    ),
)
  ...
  async with aiosqlite.connect(DB_NAME) as db:
    ...
    if count >= max_allowed:
      return await interaction.response.send_message(
          ...
          " Puedes comprar ranuras adicionales en la tienda usando"
          " `/comprar-slot` (Máximo 2 extra).",
          ephemeral=True,
      )
...
@bot.tree.command(
    name="comprar-slot",
    description=(
        "Compra 1 ranura adicional de personaje (Coste: 1000 Soles, Máx. 2"
        " extra)"
    ),
)
async def comprar_slot(interaction: discord.Interaction):
  ...
  async with aiosqlite.connect(DB_NAME) as db:
    ...
    if soles < SLOT_PRICE:
      return await interaction.response.send_message(
          f"🦊 No tienes suficientes Soles. Necesitas **{SLOT_PRICE} Soles** y"
          f" posees **{soles}**.",
          ephemeral=True,
      )
  ...
  await interaction.response.send_message(
      f"✨ Has comprado 1 ranura de personaje adicional por **{SLOT_PRICE}"
      " Soles**! Ahora puedes crear otro personaje."
  )


@bot.tree.command(
    name="perfil", description="Muestra la ficha del personaje activo de un usuario"
)
...
async def set_clan(interaction: discord.Interaction, nuevo_clan: str):
  async with aiosqlite.connect(DB_NAME) as db:
    ...
    await db.execute(
        "UPDATE characters SET clan = ? WHERE id = ?", (nuevo_clan, char[0])
    )
    await db.commit()
  await interaction.response.send_message(
      f"🦊 El clan de **{char[2]}** se ha actualizado a: **{nuevo_clan}**."
  )


# --- COMANDOS (ECONOMÍA) ---
@bot.tree.command(
    name="cuenta", description="Muestra las monedas de tu personaje activo"
)
async def cuenta(interaction: discord.Interaction):
  ...
  if not char:
    return await interaction.response.send_message(
        "🦊 No tienes un personaje activo. Usa `/crear-personaje`.",
        ephemeral=True,
    )
...
@bot.tree.command(
    name="daily",
    description="Reclama la recompensa diaria para tu personaje activo",
)
async def daily(interaction: discord.Interaction):
  ...
  async with aiosqlite.connect(DB_NAME) as db:
    ...
    if char[18] == today:
      return await interaction.response.send_message(
          "🦊 Ya has reclamado tus Soles del día. Vuelve mañana.", ephemeral=True
      )

    await db.execute(
        "UPDATE characters SET soles = soles + 50, last_daily = ? WHERE id = ?",
        (today, char[0]),
    )
    await db.commit()

  await interaction.response.send_message(
      f"🦊 Recompensa diaria entregada a **{char[2]}**: **50 Soles** 🪙."
  )


@bot.tree.command(
    name="pagar",
    description="Envía Soles desde tu personaje activo a otro jugador",
)
async def pagar(
    interaction: discord.Interaction, destinatario: discord.User, cantidad: int
):
  if cantidad <= 0:
    return await interaction.response.send_message(
  ...
  if destinatario.id == interaction.user.id:
    return await interaction.response.send_message(
        "🦊 No puedes enviarte Soles a ti mismo.", ephemeral=True
    )
  ...
  async with aiosqlite.connect(DB_NAME) as db:
    ...
    if not receiver_char:
      return await interaction.response.send_message(
          "🦊 El destinatario no tiene un personaje activo.", ephemeral=True
      )

    if sender_char[14] < cantidad:
      return await interaction.response.send_message(
          "🦊 Tu personaje no posee suficientes Soles.", ephemeral=True
      )

    await db.execute(
        "UPDATE characters SET soles = soles - ? WHERE id = ?",
        (cantidad, sender_char[0]),
    )
    await db.execute(
        "UPDATE characters SET soles = soles + ? WHERE id = ?",
        (cantidad, receiver_char[0]),
    )
    await db.commit()

  await interaction.response.send_message(
      f"🪙 **{sender_char[2]}** ha transferido **{cantidad} Soles** a"
      f" **{receiver_char[2]}** ({destinatario.mention})."
  )


# --- COMANDOS (INVENTARIO) ---
@bot.tree.command(
    name="inventario", description="Muestra la mochila de tu personaje activo"
)
async def inventario(interaction: discord.Interaction):
  ...
  if not rows:
    return await interaction.response.send_message(
        f"🦊 La bolsa de **{char[2]}** está vacía.", ephemeral=True
    )
...
@bot.tree.command(
    name="usar", description="Consume un objeto del inventario de tu personaje activo"
)
...
@bot.tree.command(
    name="craftear",
    description="Combina materiales en el inventario de tu personaje activo",
)
...
@bot.tree.command(
    name="dar-item",
    description=(
        "Otorga un ítem al personaje activo de un jugador (Master/Admin)"
    ),
)
async def dar_item(
  ...
  if cantidad <= 0:
    return await interaction.response.send_message(
        "🦊 La cantidad debe ser positiva.", ephemeral=True
    )
...
# --- COMANDOS (DADOS) ---
@bot.tree.command(
    name="tirar", description="Lanza dados (Ejemplo: 1d20+3, 2d6)"
)
...
@bot.tree.command(
    name="prueba",
    description=(
        "Prueba de atributo utilizando las estadísticas de tu personaje activo"
    ),
)
...
@bot.tree.command(
    name="catalogo", description="Ver el catálogo de una tienda"
)
...
@bot.tree.command(
    name="evento-crear", description="Anuncia un evento rápido para aventureros"
)
async def evento_crear(
  ...
  await interaction.response.send_message(
      content=f"🚨 {participantes}", embed=embed
  )


# --- COMANDOS (PERMISOS) ---
@bot.tree.command(
    name="master-rol", description="Gestiona los roles de Master"
)
@app_commands.checks.has_permissions(administrator=True)
async def master_rol(
    interaction: discord.Interaction, accion: str, rol: discord.Role
):
  async with aiosqlite.connect(DB_NAME) as db:
    if accion.lower() == "agregar":
      await db.execute(
          "INSERT OR IGNORE INTO master_roles (role_id) VALUES (?)", (rol.id,)
      )
      msg = f"🦊 Rol {rol.mention} añadido como Master."
    elif accion.lower() == "quitar":
      await db.execute(
          "DELETE FROM master_roles WHERE role_id = ?", (rol.id,)
      )
...
@bot.tree.command(
    name="tienda-rol",
    description="Gestiona los roles autorizados para abrir tienda",
)
@app_commands.checks.has_permissions(administrator=True)
async def tienda_rol(
    interaction: discord.Interaction, accion: str, rol: discord.Role
):
  async with aiosqlite.connect(DB_NAME) as db:
    if accion.lower() == "agregar":
      await db.execute(
          "INSERT OR IGNORE INTO shop_roles (role_id) VALUES (?)", (rol.id,)
      )
...
@bot.tree.command(
    name="clima", description="Muestra el estado del Abismo y del clima de hoy"
)
...
@bot.tree.command(
    name="explorar",
    description=(
        "Explora zonas en busca de materiales con tu personaje activo (3/día)"
    ),
)
async def explorar(interaction: discord.Interaction):
  ...
  async with aiosqlite.connect(DB_NAME) as db:
    ...
    if explores <= 0:
      return await interaction.response.send_message(
          "🦊 Has agotado tus 3 exploraciones del día para este personaje."
          " Vuelve mañana.",
          ephemeral=True,
      )
...
if __name__ == "__main__":
  keep_alive()  # Activa el servidor web Flask en segundo plano
  TOKEN = os.environ.get("DISCORD_TOKEN")
  if TOKEN:
    bot.run(TOKEN)
  else:
    print(
        "❌ ERROR: La variable DISCORD_TOKEN no está configurada en Render."
      )
